...
class TestTimeSeriesBestModelFinder extends AnyFlatSpec with should.Matchers with BeforeAndAfterWithContext {
  ...
  lazy val groupedData = List(Row(1d, ts), Row(2d, ts2))
  ...
  "TimeSeriesBestModelfinder" should
    ...
    val timeSeriesBestModelFinder =
      ForecastPredictor().prepareARIMAPipeline[Double]("Store", nFutures = 6)
    ...
    assert(
      head
        .asInstanceOf[ArimaModel[Double]]
        .models
        .map(_._2._2.map(_.metricResult).min)
        .min() < 1.05)
  }
  ...
  it should "receive input columns with names different from features" in {
    ...
    val structType = StructType(
      Seq(
        StructField("Store", DoubleType, nullable = false),
    ...
    val timeSeriesBestModelFinder = ForecastPredictor()
      .prepareARIMAPipeline[Double](groupByCol = "Store", labelCol = "sales", nFutures = 6)
  ...
  it should "receive input columns with names different from label in HoltWinters" in {
    ...
    val timeSeriesBestModelFinder = ForecastPredictor()
      .prepareHOLTWintersPipeline[Double](groupByCol = "store", labelCol = "sales", nFutures = 6)
    intercept[IllegalArgumentException] {
      timeSeriesBestModelFinder.fit(dataFrame)
    }
  }
  ...
  it should "model execution best value should return an root squared percentage error bellow " +
    ...
    val timeSeriesBestModelFinder =
      ForecastPredictor().prepareHOLTWintersPipeline[Double]("Store", nFutures = 6)
    val model = timeSeriesBestModelFinder.fit(dataFrame)
