...
//    val testDf = sqlContext.createDataFrame(testRdd, testStructType)
//
//    val (result,_,_,_) = ForecastPredictor().predictSmallModelFeatureBased[Double,Int, Int](trainDfDouble, testDf,
...
//  it should "execute prediction with rosenn dataset" in {
    ...
    val (bestDf,pipelineModel,groupedTest,result) = eleflow.uberdata.ForecastPredictor().
    ...
    val cachedGroupedTest = groupedTest.cache
