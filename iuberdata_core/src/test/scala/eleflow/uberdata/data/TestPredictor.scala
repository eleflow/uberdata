...
class TestPredictor extends FlatSpec with Matchers with BeforeAndAfterWithContext {
  ...
  "DataPredictor" should "throw an exception if included and excluded are empty" in {
    ...
    val structType = StructType(Seq(StructField("id", IntegerType, false), StructField("int", IntegerType, false),
      StructField("string2", StringType, false), StructField("double", DecimalType(
        ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale), false)))
  ...
  it should "select the appropriated binary algorithm" in {
    ...
    val structType = StructType(Seq(StructField("id", IntegerType, false), StructField("int", IntegerType, false),
      StructField("string2", StringType, false), StructField("double", DecimalType(
        ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale), false)))
    val testStructType = StructType(Seq(StructField("id", IntegerType, false), StructField("string2", StringType, false),
      StructField("double", DecimalType(ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale),
        false)))
    ...
    object TestPredictor extends Predictor {
      override def trainForAlgorithm(trainDataSet: RDD[LabeledPoint], algorithm: Algorithm, iterations: Int = 100) = {
    ...
    val prediction = Predictor.predictInt(schema, testSchema, target = Seq(1), includes = Seq(0, 1, 2, 3), algorithm = ToBeDetermined, iterations = 10)
  ...
  it should "select the appropriated binary algorithm logistic regression" in {
    ...
    Predictor.predictInt(schema, testSchema, target = Seq(1), includes = Seq(0, 1, 2, 3), algorithm = BinaryLogisticRegressionBFGS, iterations = 10)
  ...
  it should "test correctly load avazu test data" in {
    ...
    val structType = StructType(Seq(StructField("id", DecimalType(ClusterSettings.defaultDecimalPrecision,
      ClusterSettings.defaultDecimalScale), false), StructField("click", LongType, false),
      StructField("hour", IntegerType, false), StructField("C1", IntegerType, false),
      StructField("banner_pos", IntegerType, false), StructField("site_id", StringType, false),
      StructField("site_domain", StringType, false), StructField("site_category", StringType, false),
      StructField("app_id", StringType, false), StructField("app_domain", StringType, false),
      StructField("app_category", StringType, false), StructField("device_id", StringType, false),
      StructField("device_ip", StringType, false), StructField("device_model", StringType, false),
      StructField("device_type", IntegerType, false), StructField("device_conn_type", IntegerType, false),
      StructField("C14", IntegerType, false), StructField("C15", IntegerType, false),
      StructField("C16", IntegerType, false), StructField("C17", IntegerType, false),
      StructField("C18", IntegerType, false), StructField("C19", IntegerType, false)))
    val testType = StructType(Seq(StructField("id", DecimalType(ClusterSettings.defaultDecimalPrecision,
      ClusterSettings.defaultDecimalScale), false),
      StructField("hour", IntegerType, false), StructField("C1", IntegerType, false),
      StructField("banner_pos", IntegerType, false), StructField("site_id", StringType, false),
      StructField("site_domain", StringType, false), StructField("site_category", StringType, false),
      StructField("app_id", StringType, false), StructField("app_domain", StringType, false),
    ...
    val trainRdd = rdd.map(f => Row(f.id, f.click, f.hour, f.C1, f.banner_pos, f.site_id,
      f.site_domain, f.site_category, f.app_id, f.app_domain, f.app_category, f.device_id,
      f.device_ip, f.device_model, f.device_type, f.device_conn_type, f.C14, f.C15, f.C16,
      f.C17, f.C18, f.C19))
    val testRdd = testrdd.map(f => Row(f.id, f.hour, f.C1, f.banner_pos, f.site_id, f.site_domain, f.site_category, f.app_id, f.app_domain, f.app_category, f.device_id, f.device_ip, f.device_model, f.device_type, f.device_conn_type, f.C14, f.C15, f.C16, f.C17, f.C18, f.C19))
    ...
    Predictor.predictInt(schema, testSchema, target = Seq(1), excludes = Seq(11, 12), algorithm = ToBeDetermined, iterations = 10)
  ...
  it should "only one value can't be predicted" in {
    ...
    val trainStruct = StructType(Seq(StructField("id", IntegerType, false), StructField("int", IntegerType, false),
      StructField("string2", StringType, false), StructField("double", DecimalType(
        ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale), false)))
    val testStruct = StructType(Seq(StructField("id", IntegerType, false),
      StructField("string2", StringType, false), StructField("double", DecimalType(
        ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale), false)))
    val train = List(
      Row(1, 5, "vlr1", BigDecimal(10.5)),
    ...
    val ret : Double = 0.5
    EasyMock.expect(model.predict(EasyMock.anyObject[org.apache.spark.mllib.linalg.Vector]())).andReturn(ret).anyTimes()
    ...
    intercept[InvalidDataException] {
      TestPredictor.predictInt(trainDf, testDf, includes = Seq(0, 1, 2), algorithm = ToBeDetermined, iterations = 10)
    }
...
//  }
  ...
  it should "execute predictMultiple models" in {
    ...
    val multiplePrediction = Predictor.predictMultiple(schema,
      testSchema, target = Seq(1), excludes = Seq(11, 12), algorithm = List(BinaryLogisticRegressionSGD, LinearLeastSquares,
        NaiveBayesClassifier, BinarySupportVectorMachines), iterations = 1)
  ...
  it should "execute predictMultiple models and compare with single predict" in {
    ...
    object TestPredictor extends Predictor {
      override def splitRddInTestValidation[T](dataSet: RDD[T], train: Double = 0.7):
      (RDD[T], RDD[T]) = {
        require(train < 1d)
        require(train > 0)
    ...
    val multiplePrediction = TestPredictor.predictMultiple(schema,
      testSchema, target = Seq(1), excludes = Seq(11, 12), algorithm = List(BinaryLogisticRegressionSGD, BinarySupportVectorMachines), iterations = 10)

    val singlePrediction = TestPredictor.predict(schema,testSchema,target= Seq("click"),excludes = Seq("device_ip", "device_model"), algorithm = BinarySupportVectorMachines, iterations = 10)

    val validationMultiple = multiplePrediction.multiplePredictionValidation.get(BinarySupportVectorMachines).get
  ...
  it should "execute predictsvm" in {
    ...
    val prediction = Predictor.predictInt(schema,
      testSchema, target = Seq(1), excludes = Seq(11, 12), algorithm = BinarySupportVectorMachines, iterations = 10)
  ...
  it should "execute decisionTree" in {
    ...
    val prediction = Predictor.predictInt(schema,
      testSchema, target = Seq(1), excludes = Seq(11, 12), algorithm = DecisionTreeAlg, iterations = 10)
    val model = prediction.model
  ...
  it should "throw exception if target or id is empty" in {
    ...
    intercept[IllegalArgumentException] {
      Predictor.predictInt(schema, schema, algorithm = ToBeDetermined, iterations = 10, includes = Seq(1, 2, 3, 4), target = Seq.empty)
    }
    intercept[IllegalArgumentException] {
      Predictor.predictInt(schema, schema, algorithm = ToBeDetermined, iterations = 10, includes = Seq(1, 2, 3, 4), ids = Seq.empty)
    }
  }
  ...
  it  should "decision tree without categorization columns" in {
    ...
    val prediction = Predictor.predictInt(schema, testSchema, target = Seq(1), includes = Seq(0, 1, 3), algorithm = DecisionTreeAlg, iterations = 10)
  ...
  it should "correct execute evolutive prediction with column steps" in {
    ...
    val testrdd = sc.parallelize(testdata)
    val trainRdd = rdd.map(f => Row(f.id, f.click, f.hour, f.device_model, f.device_type,
      f.C19))
    val testRdd = testrdd.map(f => Row(f.id, f.hour, f.device_model, f.device_type,
      f.C19))
    val schema = sqlContext.applySchema(trainRdd, structType)
    val testSchema = sqlContext.applySchema(testRdd, testType)

    val result = Predictor.evolutivePredictColumnList(schema,testSchema, algorithm = BinaryLogisticRegressionBFGS,
      validationMethod = ValidationMethod.LogarithmicLoss, iterations = 1,columnNStep = List(4, 3),ids = Seq("id"),
      target = Seq("click"))
  ...
  it should "steps can't be empty" in {
    ...
    intercept[IllegalArgumentException] {
      Predictor.evolutivePredict(schema,
        schema, algorithm = BinaryLogisticRegressionBFGS, validationMethod = ValidationMethod.LogarithmicLoss, iterations = 10,
        steps = List.empty)
    }
  }
  ...
  it should "predict with columnSetup" in {
    ...
    val (_, _, _, _, _, _, correlatedColumns) = Predictor.predictWithColumnSetup(schema, testSchema, algorithm = BinaryLogisticRegressionBFGS,
      validationMethod = ValidationMethod.LogarithmicLoss, selectColumns = Left(2), iterations = 1, target = Seq
        ("click"))
  ...
  it should "excute neuralnetwork predict" in {
    ...
    val prediction = Predictor.
      predictInt(schema, testSchema, excludes = Seq(5), algorithm = ArtificialNeuralNetwork, iterations = 1)
    val model = prediction.model
  ...
  it should "naivebayes with string columns" in {
    ...
    val structType = StructType(Seq(StructField("id", IntegerType, false), StructField("int", IntegerType, false),
      StructField("string2", StringType, false), StructField("double", DecimalType(
        ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale), false)))
    val testStructType = StructType(Seq(StructField("id", IntegerType, false), StructField("int", IntegerType, false),
      StructField("double", DecimalType(ClusterSettings.defaultDecimalPrecision,ClusterSettings.defaultDecimalScale),
        false)))
    ...
    val prediction = Predictor.predictInt(schema, testSchema, target = Seq(2), includes = Seq(0, 1, 3), algorithm = ToBeDetermined, iterations = 10)
