...
/**
  ...
  */
class XGBoostBestModelFinder[L, G](
  ...
  def this()(implicit gt: ClassTag[G]) =
    this(Identifiable.randomUID("xgboost"))

  def setTimeSeriesEvaluator(eval: TimeSeriesEvaluator[G]) =
  ...
  def modelEvaluation(
    idModels: RDD[(G, Row, Seq[(ParamMap, UberXGBOOSTModel)])]
  ...
  protected def xGBoostEvaluation(
    row: Row,
    model: Booster,
    ...
    val featuresArray = row
      .getAs[Array[org.apache.spark.mllib.linalg.Vector]](
    ...
    val metric = broadcastEvaluator.value.evaluate(
      toBeValidated.map(f => (f._1.label.toDouble, f._2.toDouble))
    )
  ...
  override protected def train(dataSet: DataFrame): XGBoostModel[G] = {
    ...
    new XGBoostModel[G](uid, modelEvaluation(idModels))
      .setIdCol($(idCol))
  ...
  def train(
    id: G,
    rows: Iterator[Row]
  ): (G, Row, Seq[(ParamMap, UberXGBOOSTModel)]) = {
    val (matrixRow, result) = try {
      ...
      val valuesVector = array.map { row =>
        ...
        Vectors.dense(
          DataTransformer.toDouble(row.getAs($(labelCol))) +: vector.toArray
        )
      }
      ...
      val booster =
        UberXGBOOSTModel.fitModel(matrix, $(xGBoostParams), $(xGBoostRounds))
      (matrixRow,
       Seq(
         (new ParamMap(),
          new UberXGBOOSTModel($(xGBoostParams), $(xGBoostRounds), booster))
       ))
    } catch {
      case e: Exception =>
        log.error(
...
object XGBoostBestModelFinder
    extends DefaultParamsReadable[XGBoostBestModelFinder[_, _]] {

  override def load(path: String): XGBoostBestModelFinder[_, _] =
    super.load(path)
}
...
class XGBoostTrainingSummary[G](predictions: DataFrame,
    ...
    extends XGBoostLinearSummary[G](
      predictions,
      predictionCol,
      labelCol,
      model,
