...
/**
  ...
  */
abstract class BestModelFinder[L, M <: ForecastBaseModel[M]](
    ...
    dataSet.foreach { row =>
      ...
      if (features.size - nFutures <= 0)
        throw new IllegalArgumentException(
          s"Row ${row.toSeq.mkString(",")} has less timeseries attributes than nFutures"
        )
    }
    val data = dataSet.map { row =>
      val featuresIndex = row.fieldIndex($(featuresCol))
      val features =
        row.getAs[org.apache.spark.mllib.linalg.Vector](featuresIndex)
      val trainSize = features.size - nFutures
      val (validationFeatures, toBeValidated) =
        features.toArray.splitAt(trainSize)
      val validationRow = row.toSeq.updated(
          featuresIndex,
          Vectors.dense(validationFeatures)
  ...
  protected def arimaEvaluation(
    ...
    val features =
      row.getAs[org.apache.spark.mllib.linalg.Vector]($(featuresCol))
    log.warn(
      s"Evaluating forecast for id $id, with parameters p ${model.p}, d ${model.d} and q ${model.q}"
    )
