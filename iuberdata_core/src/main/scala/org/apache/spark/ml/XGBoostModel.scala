      ...
      case (id, ((bestModel, metrics), row)) =>
        ...
        Row(
          rowValues :+ SupportedAlgorithm.XGBoostAlgorithm.toString :+
            bestModel.params
              .map(f => f._1 -> f._2.toString) :+ forecast.head: _*
        )
    }
...
object XGBoostModel extends MLReadable[XGBoostModel[_]] {
  ...
  private[XGBoostModel] class XGBoostRegressionModelWriter(
    instance: XGBoostModel[_]
  ) extends MLWriter
      with Logging {
  ...
  private class XGBoostRegressionModelReader[G](implicit kt: ClassTag[G],
                                                ord: Ordering[G] = null)
      extends MLReader[XGBoostModel[G]] {
    ...
    override def load(path: String): XGBoostModel[G] = {
      ...
      val dataPath = new Path(path, "data").toString
      val models =
        sc.objectFile[(G, (UberXGBOOSTModel, Seq[ModelParamEvaluation[G]]))](
          dataPath
        )
