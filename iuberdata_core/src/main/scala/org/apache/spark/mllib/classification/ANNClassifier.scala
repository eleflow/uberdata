...
class ANNClassifierModel private[mllib] (
  val annModel: ArtificialNeuralNetworkModel,
  val labelToIndex: Map[Double, Int]
) extends ClassificationModel
    with ANNClassifierHelper
    with Serializable {
...
object ANNClassifier {
  ...
  def train(data: RDD[LabeledPoint],
            ...
            convergenceTol: Double): ANNClassifierModel = {
    val labelToIndex =
      data.map(lp => lp.label).distinct().collect().sorted.zipWithIndex.toMap
    new ANNClassifier(
      labelToIndex,
      hiddenLayersTopology,
      initialWeights,
