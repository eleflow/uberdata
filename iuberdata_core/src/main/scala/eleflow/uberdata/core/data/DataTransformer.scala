...
object DataTransformer {
  ...
  def createLabeledPointFromRDD(train: Dataset, test: Dataset, target: String, id: String): (RDD[((Double,
    Any), LabeledPoint)], RDD[((Double, Any), LabeledPoint)], RDD[(Int, (Int, (Any) => Int, (Any) => Double))]) =
    createLabeledPointFromRDD(train, test, Seq(target), Seq(id))

  def createLabeledPointFromRDD(train: Dataset, test: Dataset, target: Seq[String], id: Seq[String]):
  (RDD[((Double, Any), LabeledPoint)], RDD[((Double, Any), LabeledPoint)],
    RDD[(Int, (Int, (Any) => Int, (Any) => Double))]) = {
    val (newTrain, newTest) = idAndTargetAtHead(train, test, target, id)
    val summarizedColumns = prepareToSummarizeColumns(newTrain, newTest, target, id).summarizedColumns


    val columnsSize = summarizedColumns.map(_._2._1).sum().toInt
    (createLabeledPointFromRDD(newTrain, target, id, summarizedColumns, DataSetType
      .Train, columnsSize),
      createLabeledPointFromRDD(newTest, id, Seq(), summarizedColumns, DataSetType.Test, columnsSize), summarizedColumns)
  }

  //TODO
  def idAndTargetAtHead(train: Dataset, test: Dataset, target: Seq[String], id: Seq[String]) = {
    val newTrain = if (train.columnIndexOf(target.head) == 1 && train.columnIndexOf(id.head) == 0) {
      train
    } else {
  ...
  def createLabeledPointFromRDD(dataset: Dataset, target: Seq[String], id: Seq[String],
  ...
  def createLabeledPointFromRDD(dataset: Dataset, target: Seq[String], id: Seq[String], datasetType: DataSetType.Types): RDD[
    ((Double, Any), LabeledPoint)] = {
    createLabeledPointFromRDD(dataset, target, id, dataset.sliceByName(excludes = id ++ target).summarizedColumns, datasetType, dataset.columnsSize
      .toInt - target.size - id.size)
  }

  def mapIntIdsToString(rdd: DataFrame, columns: Seq[Int]): Seq[String] = rdd.schema.fields.zipWithIndex.filter {
    case (field, index) => columns.contains(index)
  }.map(_._1.name)

  def extractColumnsFromLP(columns: Array[(Double, Int)], train: RDD[LabeledPoint],
                           validation: RDD[LabeledPoint], test: RDD[((Double, Any), LabeledPoint)]):
  ...
  def extractColumns(columnsIndex: mutable.IndexedSeq[Int], train:
  RDD[LabeledPoint]) = {
    train.map(extractLP(columnsIndex)(_))
  }

  def extractLP(columnsIndex: mutable.IndexedSeq[Int]) = {
    lp: LabeledPoint => val featureArray = lp.features.toArray
      val values = columnsIndex.map {
        index => featureArray(index)
      }.zipWithIndex.filter(_._1 != 0d).toArray
      val (attributes, indexes) = values.unzip
      val vector = Vectors.sparse(columnsIndex.size, indexes.toArray, attributes.toArray)
