...
class ForecastPredictor extends Serializable with Logging {
  ...
  protected def defaultARIMAParamMap[T <: ArimaParams](
    estimator: T,
  ...
  protected def prepareARIMAPipelineInt[L](
    ...
    val paramGrid =
      defaultARIMAParamMap[ArimaBestModelFinder[L]](arima, paramRange)
    val arimaBestModelFinder: ArimaBestModelFinder[L] =
      arima.setEstimatorParamMaps(paramGrid)
  ...
  private def preparePipeline(
    timeSeriesBestModelFinder: TimeSeriesBestModelFinder,
    preTransformers: Array[_ <: Transformer]
  ): Pipeline = {
  ...
  def prepareBestForecastPipeline[L](
    ...
    val paramGrid =
      defaultARIMAParamMap[ForecastBestModelFinder[L, ForecastBestModel[L]]](
        findBestForecast,
        paramRange
      )
  ...
  def smallModelPipelineStages(labelCol: String,
    ...
    val stringColumns = schema
      .filter(
        f =>
          f.dataType.isInstanceOf[StringType] && featuresCol.contains(f.name)
      )
      .map(_.name)
    ...
    val assembler = new VectorAssembler()
      .setInputCols(
        stringColumns.map(f => s"${f}Index").toArray :+ "nonStringIndex"
      )
  ...
  def predict[L, G](
    ...
    algorithm match {
      case Arima | HoltWinters | MovingAverage8 | MovingAverage16 |
          MovingAverage26 | FindBestForecast =>
  ...
  def predictSmallModelFeatureBased[L, G](
    ...
    val pipeline = prepareXGBoost[L, G](
      labelCol,
      featuresCol,
      validationCol,
      timeCol,
    ...
    val joined =
      result.select(idCol, IUberdataForecastUtil.FEATURES_PREDICTION_COL_NAME)
    val dfToBeReturned =
      joined.withColumnRenamed("featuresPrediction", "prediction")

    (dfToBeReturned.sort(idCol), model)
  }

  def predictSmallModelFuture[L](
    train: DataFrame,
    test: DataFrame,
    labelCol: String,
    featuresCol: String,
    ...
    ord: Ordering[L] = null): (DataFrame, PipelineModel) = {
    require(
      algorithm != XGBoostAlgorithm,
    ...
    val pipeline = algorithm match {
      ...
      case MovingAverage8 =>
        prepareMovingAveragePipeline[L](
          labelCol,
          featuresCol,
          validationCol,
          timeCol,
    ...
    val labelColIndexBc =
      sparkContext.broadcast(result.columns.indexOf(labelCol))
    ...
    val groupedTest = test.rdd
      .groupBy(row => row.getAs[L](labelColBc.value))
      .map {
        case (key, values) =>
          val sort = values.toArray.map { row =>
            IUberdataForecastUtil.convertColumnToLongAddAtEnd(
              row,
              timeColIndexBc.value
            )
      ...
      case (key, ((predictions, row), ids)) =>
        ...
        ids.zip(predictions).map {
          case (id, prediction) =>
            ...
            Row(
              used ++ filteredRow.map { case (_, index) => row.get(index) } :+ Math
                .round(prediction): _*
            )
        }
    }
    ...
    val schema = result.schema.fields
      .filter(
        f =>
          f.name != validationCol && f.name != labelCol && f.name != "features"
