...
public class SparkDependencyResolver extends AbstractDependencyResolver {
    ...
    private void loadFromFs(String artifact, boolean addSparkContext) throws Exception {
        ...
        if (addSparkContext) {
            uc.sparkContext().addJar(jarFile.getAbsolutePath());
        }
    }
    ...
    private List<String> loadFromMvn(String artifact, Collection<String> excludes,
        ...
        if (addSparkContext) {
            Set<String> jars = files.stream().map(f -> f.getAbsolutePath()).collect(Collectors
                    .toSet());

            ClusterSettings.jarsToBeAdded_$eq(JavaConverters.asScalaSetConverter
                    (jars).asScala().toSet());
        }
