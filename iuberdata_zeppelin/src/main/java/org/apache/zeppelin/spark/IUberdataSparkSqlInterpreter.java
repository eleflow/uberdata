 ...
 */
public class IUberdataSparkSqlInterpreter extends Interpreter {
  Logger logger = LoggerFactory.getLogger(IUberdataSparkSqlInterpreter.class);
  ...
  static {
    Interpreter.register(
            "sql",
            "iuberdata",
            IUberdataSparkSqlInterpreter.class.getName(),
            new InterpreterPropertyBuilder()
                    .add("zeppelin.spark.maxResult",
                            SystemDefaultUtil.getSystemDefault("ZEPPELIN_SPARK_MAXRESULT",
                                    "zeppelin.spark.maxResult", "1000"),
                            "Max number of SparkSQL result to display.")
                    .add("zeppelin.spark.concurrentSQL",
                            SystemDefaultUtil.getSystemDefault("ZEPPELIN_SPARK_CONCURRENTSQL",
  ...
  public IUberdataSparkSqlInterpreter(Properties property) {
    super(property);
  }
  ...
  private IUberSparkInterpreter getSparkInterpreter() {
    ...
    Interpreter p = getInterpreterInTheSameSessionByClassName(IUberSparkInterpreter.class.getName());
  ...
  public InterpreterResult interpret(String st, InterpreterContext context) {
    ...
    SparkVersion sparkVersion = sparkInterpreter.getSparkVersion();
    if (sparkVersion != null && sparkVersion.isUnsupportedVersion()) {
      return new InterpreterResult(Code.ERROR, "Spark "
              + sparkInterpreter.getSparkVersion().toString() + " is not supported");
    }

      if(!getSparkInterpreter().isSparkContextInitialized()) {
          return new InterpreterResult(Code.ERROR, "Spark context isn't initialized, please start it using " +
                  "uc.sparkContext before using %sql interpreter");
      }
  ...
  public Scheduler getScheduler() {
    if (concurrentSQL()) {
      ...
      return SchedulerFactory.singleton().createOrGetParallelScheduler(
              IUberdataSparkSqlInterpreter.class.getName() + this.hashCode(), maxConcurrency);
    } else {
      ...
      Interpreter intp =
              getInterpreterInTheSameSessionByClassName(IUberSparkInterpreter.class.getName());
      if (intp != null) {
