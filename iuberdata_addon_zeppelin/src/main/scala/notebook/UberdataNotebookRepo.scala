...
class UberdataNotebookRepo(conf: ZeppelinConfiguration) extends VFSNotebookRepo(conf) {
  private val log: Logger = LoggerFactory.getLogger(classOf[UberdataNotebookRepo])
  ...
  private val renameOldRepos = {
    ...
    val allNotes = list()
    allNotes.filter {
      notebook => notebook.getName != notebook.getId && !resolveFile(notebook.getName).exists()
    }.foreach {
      notebook =>
        ...
        val noteFile: FileObject = noteFolder.resolveFile("note.json", NameScope.CHILD)
        val json: String = gson.toJson(getNote(oldRepo))
        val out: OutputStream = noteFile.getContent.getOutputStream(false)
        out.write(json.getBytes(conf.getString(ConfVars.ZEPPELIN_ENCODING)))
        out.close
        localNotes.filter(f => f._2 != notebook.getName && f._1 == notebook.getId()).map {
          f =>
  ...
  @throws(classOf[IOException])
  override def list(subject: AuthenticationInfo = ???): java.util.List[NoteInfo] = {
    import scala.collection.JavaConversions._

    children.filter {
      f =>
        val fileName: String = f.getName.getBaseName
        !(f.isHidden || fileName.startsWith(".") || fileName.startsWith("#") || fileName.startsWith("~") || !isDirectory(f))
    }.map {
      case f =>
        try {
          getNoteInfo(f)
  ...
  private def getChildren(rootDir: FileObject): Array[FileObject] = {
    if (isDirectory(rootDir)) {
      ...
      val (folders, _) = children.partition {
        f => isDirectory(f)
      }
      val x = folders.headOption.map(_ =>
        folders flatMap (getChildren(_))
  ...
  override def get(noteId: String, subject: AuthenticationInfo): Note = getNote(noteId).map(getNote(_)).headOption.getOrElse(
    throw new Exception("Notebook not found"))

  private def getNote(noteId: String) = {
    import scala.collection.JavaConversions._
    list().filter(f => f.getId == noteId).map {
      f =>
  ...
  private def getNote(noteDir: FileObject): Note = {
    ...
    val noteJson: FileObject = noteDir.resolveFile("note.json", NameScope.CHILD)
    if (!noteJson.exists) {
    ...
    val json: String = IOUtils.toString(ins, conf.getString(ConfVars.ZEPPELIN_ENCODING))
    ins.close
  ...
  override def save(note: Note, subject: AuthenticationInfo): Unit = if (note.getName != null) {
    val localNotes = notebooks
    val pattern = s"""${note.getName}\\-V(\\d{1,10})""".r
    val sameNameNotes = localNotes.filter { f =>
      (f._2 == note.getName || f._2.startsWith(note.getName) && pattern.findFirstIn(f._2).isDefined) && f._1 != note.id()
    }
    sameNameNotes.headOption.map {
      _ => note.setName(s"${note.getName}-V${sameNameNotes.size + 1}")
    }
  ...
  private def getPath(path: String) = if (path == null || path.trim.length == 0) {
  ...
  private def isDirectory(fo: FileObject) = fo != null && fo.getType() == FileType.FOLDER
}
